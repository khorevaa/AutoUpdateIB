...
Функция ПолучитьСписокШагов(КонтекстФреймворкаBDD) Экспорт
	...
	ВсеШаги.Добавить("ЯСоздаюОчередьобновленияrmq");
	ВсеШаги.Добавить("ЯУстанавливаюНастройкиОчередьобновленияrmqСерверПортПользовательПарольВиртуальныйХостОчередь");
...
//Я создаю ОчередьОбновленияRMQ
Процедура ЯСоздаюОчередьобновленияrmq() Экспорт
	ОчередьОбновленияRMQ = Новый ОчередьОбновленияRMQ;
	БДД.СохранитьВКонтекст("ОчередьОбновленияRMQ", ОчередьОбновленияRMQ);
КонецПроцедуры

//Я устанавливаю настройки ОчередьОбновленияRMQ сервер "localhost" порт "8080" пользователь "guest" пароль "guest" виртуальный хост "%2f" очередь "all.update"
Процедура ЯУстанавливаюНастройкиОчередьобновленияrmqСерверПортПользовательПарольВиртуальныйХостОчередь(Знач АдресСервера, Знач ПортСервера, Знач ПользовательRMQ, Знач ПарольRMQ, Знач ВиртуальныйХост, Знач ИмяОчереди) Экспорт
	...
	НастройкаRMQ.Вставить("ТочкаОбмена", "");

	ОчередьОбновленияRMQ = БДД.ПолучитьИзКонтекста("ОчередьОбновленияRMQ");
	ОчередьОбновленияRMQ.Инициализировать(НастройкаRMQ);
