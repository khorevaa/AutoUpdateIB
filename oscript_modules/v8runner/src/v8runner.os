...
Функция ЗапуститьИПодождать(Знач Параметры)
	...
	Если мПризнакОжиданияВыполненияПрограммы Тогда
		...
		Если НЕ мОбработчикОжиданияПроцессаGUI = Неопределено Тогда
			Команда.УстановитьОбработчикОжидания(мОбработчикОжиданияПроцессаGUI.Объект, мОбработчикОжиданияПроцессаGUI.Интервал);
		КонецЕсли;
...
Процедура УстановитьВывод(Знач Сообщение)
	Если мОчищатьФайлИнформации
		...
		мВыводКоманды = СокрЛП( Сообщение );
	Иначе
