...
Функция ПроверитьВозможностьПрименения(Расширение, ДвоичныеДанныеРасширения = Неопределено, НаименованиеРасширения = "", ВерсияРасширения = "0.0.0"); 
	...
	Если ЕстьКритичныеОшибки(МассивОшибок) Тогда
		...
		КритичнаяОшибка("{%1:%2}: Применить расширение <%3> - невозможно ", НаименованиеРасширения, ВерсияРасширения, СинонимРасширения);
		Возврат Ложь;
...
Функция ЕстьКритичныеОшибки(МассивОшибокПодключения)
	
	Для каждого ИнформацияОПроблемеПримененияРасширения Из ПроблемsПримененияРасширения Цикл
