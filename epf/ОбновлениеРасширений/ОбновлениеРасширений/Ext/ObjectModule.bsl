...
Функция ПодготовитьЛог() 
	...
	ЗаписьJSON.ОткрытьФайл(ПутьКФайлуЛогаОбовления,,, ПараметрыЗаписиJSON);
...
Процедура ИзменитьРасширение(РасширениеКонфигурации, СтрокаРасширения)
	...
	Если ЗначениеЗаполнено(СтрокаРасширения._ПутьКФайлуРасширения) Тогда
		...
		Информация("Обноеление конфигурации расширение <%1>", НаименованиеРасширения);
	...
	Если РасширениеКонфигурации.ИзменяетСтруктуруДанных() Тогда
		 ...
		 Информация("Расширение <%1> изменяет метаданные базы", НаименованиеРасширения);
	
	КонецЕсли; 
	
	
	Если ПроверитьВозможностьПрименения(РасширениеКонфигурации, ДвоичныеДанныеРасширения, НаименованиеРасширения) Тогда
		
		Информация("Записываю в базу изменения расширения <%1>", НаименованиеРасширения);
...
Функция ПроверитьВозможностьПрименения(Расширение, ДвоичныеДанныеРасширения = Неопределено, НаименованиеРасширения = "") 
	
	НаименованиеРасширения = ?(ЗначениеЗаполнено(Расширение.Синоним), Расширение.Синоним, НаименованиеРасширения);
	
	Информация("Выполняю проверку на возможность применения расширения <%1>", НаименованиеРасширения);
	
	МассивОшибок = Расширение.ПроверитьВозможностьПрименения(ДвоичныеДанныеРасширения);
	Если Не МассивОшибок.Количество() = 0 Тогда
		Информация("Применить расширение <%1> - невозможно ", НаименованиеРасширения);
		ПоказатьРезультатыПроверки(МассивОшибок);
		Возврат Ложь;
	Конецесли;
	
	Информация("Проверка возможности применения расширения <%1> - пройдена", НаименованиеРасширения);
	
	Возврат Истина;
	
КонецФункции

Procedure ПоказатьРезультатыПроверки(Знач ПроблемsПримененияРасширения)
	
	For Each ИнформацияОПроблемеПримененияРасширения in ПроблемsПримененияРасширения Do
		НаименованиеРасширения = ?(ЗначениеЗаполнено(ИнформацияОПроблемеПримененияРасширения.Расширение.Синоним),
...
Процедура ДополнитьТаблицуРасширений()
	...
	Для Каждого ОписаниеРасширения Из НаборРасширений Цикл
		...
		Если СтрокаТаблицаРасширения = Неопределено Тогда
			...
			ПутьКФайлуРасширения = Неопределено;
			
			Если ОписаниеРасширения.Свойство("ПутьКФайлуРасширения", ПутьКФайлуРасширения)
				И ЗначениеЗаполнено(ПутьКФайлуРасширения) Тогда
				...
				НоваяСтрока._ПутьКФайлуРасширения = ПутьКФайлуРасширения;
...
Процедура ОпределитьИзмененияРасширения(СтрокаТаблицаРасширения, ОписаниеРасширения)
	...
	Если ОписаниеРасширения.Свойство("ПутьКФайлуРасширения", ПутьКФайлуРасширения)
		...
		СтрокаТаблицаРасширения._Изменить = Истина;
		СтрокаТаблицаРасширения._ПутьКФайлуРасширения = ПутьКФайлуРасширения;
	...
	Если ОписаниеРасширения.Свойство("БезопасныйРежим", БезопасныйРежим)
		И ЗначениеЗаполнено(БезопасныйРежим)
		И НЕ БезопасныйРежим = СтрокаТаблицаРасширения.БезопасныйРежим Тогда
...
Функция ОбработатьТаблицуРасширений()
	...
	Для каждого СтрокаРасширения Из ТаблицаРасширений Цикл
		...
		Если СтрокаРасширения._Удалить Тогда
			...
			Информация("Удаление расширения <%1> (Версия: <%2>)", СтрокаРасширения.Синоним, ВерсияРасширения); 
		...
		ИначеЕсли СтрокаРасширения._Отключить Тогда
			...
			Информация("Отключаю расширения <%1> (Версия: <%2>)", СтрокаРасширения.Синоним, ВерсияРасширения); 
		...
		ИначеЕсли СтрокаРасширения._Добавить Тогда
			...
			Информация("Добавляю новое расширения <%1>", СтрокаРасширения.Имя); 
		...
		ИначеЕсли СтрокаРасширения._Изменить Тогда
			...
			Информация("Изменяю расширения <%1> (Версия: <%2>)", СтрокаРасширения.Синоним, ВерсияРасширения); 
			РасширениеКонфигурации  = НайтиУстановленноеРасширениеПоИмени(СтрокаРасширения.Имя);
			ИзменитьРасширение(РасширениеКонфигурации, СтрокаРасширения); 
...
Процедура НачатьРаботу() Экспорт
	...
	ПутьКФайлуЛогаОбовления = МассивПараметров[1];
