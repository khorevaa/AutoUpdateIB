...
Процедура ПрерватьПодключения()
	...
	ТрассировкаЗадач = Новый ТрассировкаЗадач(Лог);
	ТрассировкаЗадач.Трассировать("Отключение сеансов подключений", "Прерываю сеансы и подключения",,, ДопПоля);
	...
	Для Сч = 1 По ЧислоПопыток Цикл
		...
		Попытка
			...
			Если ЧислоПопыток = Сч Тогда
				ВызватьИсключение Новый ИнформацияОбОшибке("Не удалось прервать сеансы и соединения", Соединения.Количество());
			КонецЕсли;
		...
		Исключение
			...
			Если ЧислоПопыток = Сч Тогда
				...
				ТрассировкаЗадач.Трассировать("Отключение сеансов подключений", "Не удалось прервать соединения и сеансы в информационной базе","error", ИнформацияОбОшибке(), ДопПоля);
	...
	ТрассировкаЗадач.Трассировать("Отключение сеансов подключений", "Прерваны сеансы и подключения в информационной базе","success", , ДопПоля);
