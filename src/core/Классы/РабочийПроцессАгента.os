...
Процедура Запустить(Знач ПутьКФайлуСообщения) Экспорт
	...
	ДанныеСообщенияRMQ = ПолучитьДвоичныеДанныеИзBase64Строки(ДанныеРабочегоПроцесса["ДанныеСообщенияRMQ"]);
	
	ВходящееСообщениеRMQ = Новый СообщениеRMQ;

	Лог.Отладка("Читаю данные сообщения RMQ <%1>", ПолучитьСтрокуИзДвоичныхДанных(ДанныеСообщенияRMQ));

	ВходящееСообщениеRMQ.ИзJson(ПолучитьСтрокуИзДвоичныхДанных(ДанныеСообщенияRMQ));
	...
	Лог.Отладка("Читаю данные сообщения");
