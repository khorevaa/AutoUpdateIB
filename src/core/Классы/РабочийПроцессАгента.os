...
Процедура Запустить(Знач ПутьКФайлуСообщения) Экспорт
	...
	КаталогВременныхФайловПроцессов = ОбъединитьПути(КаталогЛогов, "temp", Идентификатор());

	ФС.ОбеспечитьКаталог(КаталогВременныхФайловПроцессов);

	УстановитьПеременнуюСреды("TEMP", КаталогВременныхФайловПроцессов);
	УстановитьПеременнуюСреды("Temp", КаталогВременныхФайловПроцессов);
	УстановитьПеременнуюСреды("Tmp", КаталогВременныхФайловПроцессов);
	...
	ПараметрыОбновления = РаботаСФайлами.ОбъектИзJson(ДанныеСообщения);
	...
	Попытка
		...
		УдалитьФайлы(КаталогВременныхФайловПроцессов);
	Исключение

		Лог.Отладка("Ошибка удаления временных файлов <%1>", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
