...
Функция ДобавитьФайл(Знач ПутьКФайлу, Знач ХешСумма, Знач ИмяХешФункции = "MD5", Знач ОжиданиеВозможности = Ложь) Экспорт
	...
	ПроверочнаяХешСумма = ПолучитьХешФайла(Новый ДвоичныеДанные(ФайлДобавляемогоФайла.ПолноеИмя), ИмяХешФункции);
	Если НЕ ПроверочнаяХешСумма = ХешСумма Тогда
		ВызватьИсключение СтрШаблон("Не совпадает хеш сумма <%1> для переданного файла. Добавление в индекс невозможно", ПутьКФайлу);
	КонецЕсли;
...
Процедура ЗаписатьИндексКеша()
	...
	Для каждого ХешФайла Из ИндексХешаФайлов Цикл
...
Процедура ОписаниеФайлаКеша(Знач ИмяФайла, Знач РасширениеФайла, Знач ХешСумма, Знач Подпись)
	
	ОписаниеФайла = Новый Структура();
	ОписаниеФайла.Вставить("ИмяФайла", ИмяФайлаКеша);
...
Функция ПолучитьДатуФайлаКеша()
	...
	ФайлКеша = Новый Файл(ПутьКФайлуКеша);
...
Процедура ОписаниеФайлаКешаДляЗаписи(Знач ИмяФайла, Знач РасширениеФайла, Знач ХешСумма, Знач Подпись)
	
	ОписаниеФайла = Новый Структура();
	ОписаниеФайла.Вставить("name", ИмяФайлаКеша);
...
Функция ПодписатьХеш(Знач ХешСумма)
	...
	ПодписанныеДанные = Шифрование.HMAC(ДвоичныеДанныеПодписи, ХешСумма, "SHA256");
...
Процедура Разблокировать(Знач ИдентификаторПроцесса) 
	...
	ИдентификаторПроцессаИзФайла = ПрочитатьФайл(ПутьКФайлуБлокировкиКеша);
	
	Если НЕ ИдентификаторПроцесса = ИдентификаторПроцессаИзФайла Тогда
		ВызватьИсключение "Файл кеша заблокирован другим процессом";
	КонецЕсли;
