...
Функция Получить(ОбработчикПолучения) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ХешФайла) Тогда
		Возврат ПолучитьФайлЧерезОбработчик(ОбработчикПолучения);
	КонецЕсли;
	...
	Если ПутьКФайлу = Неопределено Тогда
		...
		ПутьКФайлу = ПолучитьФайлЧерезОбработчик(ОбработчикПолучения);
...
Функция ПолучитьФайлЧерезОбработчик(ОбработчикПолучения)
	
	ОбработчикПолучения.Получить(КаталогКешаФайлов);

	ОбновитьКеш();

	ПутьКФайлу = ИндексХешаФайлов[ОбработчикПолучения.ХешФайла()];

	Если ПутьКФайлу = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Ошибка получения файла по хешу <%1>. Файл не существует", ОбработчикПолучения.ХешФайла());
	КонецЕсли;
