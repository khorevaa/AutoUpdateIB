...
Процедура ОтправитьИнформацииОНачале(ДанныеНаРабочийПроцесс, ИдентификаторРабочегоПроцесса)
	...
	ДанныеПоСистеме.Вставить("ПользовательОС", ПользовательОС);
	ДанныеПоСистеме.Вставить("ИмяКомпьютера", ИмяКомпьютера);
	ДанныеПоСистеме.Вставить("ИдентификаторРабочегоПроцесса", ИдентификаторРабочегоПроцесса);
	ДанныеПоСистеме.Вставить("Стартован", Истина);

	ПараметрыСообщения = Новый Структура;
	ПараметрыСообщения.Вставить("correlation_id", ДанныеНаРабочийПроцесс["КлючСообщения"]);
	ПараметрыСообщения.Вставить("timestamp", Цел(Формат(ТекущаяДата() - дата(1970,1,1,1,0,0), "ЧГ=0")));

	ОчередьRMQ.ОтправитьДанныеВОчередь(ДанныеПоСистеме,
						ПараметрыСообщения,
