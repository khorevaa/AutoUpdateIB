...
Функция ПреобразоватьВПараметрыЗапуска(Параметры)
	...
	ДобавитьПараметрВМассив(ПараметрыЗапуска, "--user", НастройкаRMQ.Пользователь);
	ДобавитьПараметрВМассив(ПараметрыЗапуска, "--password", НастройкаRMQ.Пароль);
	ДобавитьПараметрВМассив(ПараметрыЗапуска, "--queue", НастройкаRMQ.ИмяОчереди);
	ДобавитьПараметрВМассив(ПараметрыЗапуска, "--exchange-name", НастройкаRMQ.ТочкаОбмена);
	ДобавитьПараметрВМассив(ПараметрыЗапуска, "--virtual-host", НастройкаRMQ.ВиртуальныйХост);

	ДобавитьПараметрВМассив(ПараметрыЗапуска, "--server", НастройкаRMQ.Сервер);
	ДобавитьПараметрВМассив(ПараметрыЗапуска, "--port", НастройкаRMQ.Порт);
...
Процедура ЗапуститьПаралельно(КоличествоПроцессов = 0)
	...
	Для Счетчик = 1 По КоличествоПроцессов - КоличествоРаботающихПроцессов Цикл
		...
		ДанныеНаРабочийПроцесс = ОчередьRMQ.ПолучитьИзОчереди();

		Если ДанныеНаРабочийПроцесс = Неопределено Тогда
			Возврат;
		КонецЕсли;

		Лог.Отладка("Тип сообщения <%1>", ТипЗнч(ДанныеНаРабочийПроцесс));
		Лог.Отладка("Ключ сообщения <%1>", ДанныеНаРабочийПроцесс["КлючСообщения"]);
		...
		ОтправитьИнформацииОНачале(ДанныеНаРабочийПроцесс, ИдентификаторРабочегоПроцесса);

		ИмяФайлаСообщения = ПодготовитьФайлСообщенияНаРабочийПроцесс(ДанныеНаРабочийПроцесс);
...
Функция ПодготовитьФайлСообщенияНаРабочийПроцесс(ДанныеНаРабочийПроцесс)
...
Процедура ОтправитьИнформацииОНачале(ДанныеНаРабочийПроцесс, ИдентификаторРабочегоПроцесса)

	ДанныеОНачале = Новый Структура;
	ДанныеОНачале.Вставить("ПользовательОС", ПользовательОС);
	ДанныеОНачале.Вставить("ИмяКомпьютера", ИмяКомпьютера);
	ДанныеОНачале.Вставить("ИдентификаторРабочегоПроцесса", ИдентификаторРабочегоПроцесса);
	ДанныеОНачале.Вставить("Стартован", Истина);
	ДанныеОНачале.Вставить("ДатаВремя", ТекущаяДата());


	ПараметрыСообщения = Новый Структура;
	ПараметрыСообщения.Вставить("correlation_id", ДанныеНаРабочийПроцесс["КлючСообщения"]);
	ПараметрыСообщения.Вставить("timestamp", РаботаСДатой.ТекущаяДатаВМеткуВремени());

	ОчередьRMQ.ОтправитьДанныеВОчередь(ДанныеОНачале,
						ПараметрыСообщения,
						ДанныеНаРабочийПроцесс["КлючМаршрутизации"]);
