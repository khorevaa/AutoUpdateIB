...
Функция ДобавитьФайл(Знач ПутьКФайлу, Знач ХешСумма, Знач ИмяХешФункции = "MD5", Знач ОжиданиеВозможности = Истина) Экспорт
	...
	Лог.Отладка("Вычисляю хеш файла <%1> и сравниваю с переданным", ФайлДобавляемогоФайла.ПолноеИмя);
	ДвоичныеДанныеФайла = Новый ДвоичныеДанные(ФайлДобавляемогоФайла.ПолноеИмя);
	ПроверочнаяХешСумма = ПолучитьХешФайла(ДвоичныеДанныеФайла, Строка(ИмяХешФункции));
	...
	Попытка
		...
		ИндексКешаФайлов.Вставить(ХешСумма, ОписаниеФайлаКеша(ИмяФайла, РасширениеФайла, ХешСумма, ПодписьХешСуммы, ИмяХешФункции));
		ЗаписатьИндексКеша();
...
Процедура ПрочитатьИндексХешСумм()
	...
	Для каждого ЭлементИндекса Из ДанныеИндекса Цикл
		...
		ОписаниеФайла = ОписаниеФайлаКеша(ЭлементИндекса["name"], ЭлементИндекса["ext"], ЭлементИндекса["hash"], ЭлементИндекса["sig"], ЭлементИндекса["func"]);
	...
	Если Заблокирован() Тогда
		...
		ПодождатьРазблокировки(5*60);

	КонецЕсли;

	ИдентификаторПроцесса = Новый УникальныйИдентификатор();

	Заблокировать(ИдентификаторПроцесса);

	ПроверитьХешФайлов();

	ВыполнитьСборкуМусора();

	Разблокировать(ИдентификаторПроцесса);

КонецПроцедуры

Процедура ПроверитьХешФайлов()
	
	МассивОчисткиДанных = Новый Массив();

	Для каждого ДанныеИндекса Из ИндексКешаФайлов Цикл
		
		ХешСумма = ДанныеИндекса.Ключ;
		ОписаниеФайлаКеша = ДанныеИндекса.Значение;
		ПутьКФайлу = ПутьКФайлуКеша(ХешСумма, ОписаниеФайлаКеша.РасширениеФайла);
		ИмяХешФункции = ОписаниеФайлаКеша.ХешФункция;
		ДвоичныеДанные = Новый ДвоичныеДанные(ПутьКФайлу);
		ПосчитанныйХешФайла = ПолучитьХешФайла(ДвоичныеДанные, ИмяХешФункции);
		
		ОсвободитьОбъект(ДвоичныеДанные);
		
		ДвоичныеДанные = Неопределено;
		Если Не НРег(ПосчитанныйХешФайла) = НРег(ХешСумма) Тогда
			МассивОчисткиДанных.Добавить(ХешСумма);
		КонецЕсли;

	КонецЦикла;

	Лог.Отладка("Удаляю устаревшие файлы из кеша. Количество <%1>", МассивОчисткиДанных.Количество());
	Для каждого ЭлементМассива Из МассивОчисткиДанных Цикл
		
		ИндексКешаФайлов.Удалить(ЭлементМассива);
...
Функция ОписаниеФайлаКеша(Знач ИмяФайла, Знач РасширениеФайла, Знач ХешСумма, Знач Подпись, Знач ХешФункции)
	...
	ОписаниеФайла.Вставить("ХешФункция", ХешФункции);
...
Функция ОписаниеФайлаКешаДляЗаписи(Знач ИмяФайла, Знач РасширениеФайла, Знач ХешСумма, Знач Подпись, Знач ХешФункции)
	...
	ОписаниеФайла.Вставить("func", ХешФункции);
	
	Возврат ОписаниеФайла;
	
КонецФункции

Функция ПолучитьХешФайла(Знач ДвоичныеДанныеФайла, Знач ИмяХешФункции = "MD5")
...
Процедура ПриСозданииОбъекта(Знач ПутьККаталогуКеша)
	...
	Лог = Логирование.ПолучитьЛог("oscript.app.lib.cache");
