...
Процедура ИзСоответствия(ВходящееСоответствие) Экспорт
	...
	Иначе
		...
		ДанныеСообщения = РаскодироватьСтроку(ВходящиеДанныеСообщения, СпособКодированияСтроки.КодировкаURL);
...
Функция ВJSON() Экспорт
	...
	ТекстJSON = ОбъектВJson(ОбъектСериализации);
	
	Возврат ТекстJSON;
	
КонецФункции

Функция ОбъектВJson(Знач Объект)
