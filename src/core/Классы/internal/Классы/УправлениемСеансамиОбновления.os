...
Процедура УстановитьБлокировкуИнформационнойБазы()
	...
	ПараметрыИнформационнойбазы.Вставить("ВремяБлокировки", Формат(НачалоБлокировкиСеансов, ФорматДаты));
...
Процедура СнятьБлокировкуСеансов()
	...
	Лог.Информация("Сеансы разрешены");
...
Процедура УстановитьБлокировкуСеансов(Знач ПараметрыБлокировки)
	...
	Лог.Информация("Сеансы запрещены");
...
Функция НайтиБазуВКластере(ИмяБазыДанных)
	...
	ПараметрыЗапуска = Новый Массив();
	ПараметрыЗапуска.Добавить("infobase summary list");
	ПараметрыЗапуска.Добавить(КлючИдентификатораКластера());
...
Функция ВыполнитьВИнформационнойБазе(Знач ПараметрыКоманды)
	...
	ПараметрыКоманды.Добавить("--infobase=""%1""", ИдентификаторБазы());
...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды, Знач БезКластера = Ложь)
	...
	КомандаВыполнения = СтрокаЗапускаКлиента();
	АдресСервераАдминистрирования = УправлениеКластером.АдресСервераАдминистрирования;
...
Функция ПолучитьСписокСеансов()
	...
	СписокСеансовИБ = ВыполнитьВИнформационнойБазе(ПараметрыЗапуска);	
...
Процедура ОтключитьСеанс(Знач Сеанс)
	...
	ПараметрыЗапуска.Добавить(СтрШаблон("--session=""%3""", Сеанс.Идентификатор));
