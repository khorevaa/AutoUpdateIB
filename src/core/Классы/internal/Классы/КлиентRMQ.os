...
Функция ОтправитьСообщениеRMQ(Знач СообщениеRMQ) Экспорт
	...
	Лог.Отладка("Тело сообщения <%1>", РаботаСФайлами.ОбъектВJson(ТелоЗапроса));
	...
	Если Не ДанныеОтвета["routed"] = Истина Тогда
		...
		ВызватьИсключение СтрШаблон("Не удалось отправить данные в точку обмена <%1>, ключ маршрутизации <%2>, данные сообщения <%3>",
					 СообщениеRMQ.ТочкаОбмена, СообщениеRMQ.КлючМаршрутизации, СообщениеRMQ.ДанныеСообщения);
	КонецЕсли;
...
Процедура УстановитьНастройкиПодключения(Знач НовыеНастройкиПодключения) Экспорт
	...
	Аутентификация(НастройкиПодключения.Пользователь, НастройкиПодключения.Пароль);
...
Процедура ПоказатьОшибкуRMQ(Знач ОтветRMQ)
	...
	Лог.Поля("error", ОтветRMQ["error"]).Ошибка("Ошибка RMQ: %1", РаботаСФайлами.ОбъектВJson(ОтветRMQ));
...
Процедура ПриСозданииОбъекта(Знач ПАдресСервера = "", Знач ПВиртуальныйХост = "%2F")
	...
	ПараметрыЗапроса.Вставить("Аутентификация", Аутентификация);
	ПараметрыЗапроса.Вставить("ПараметрыЗаписиJSON", ПараметрыЗаписиJSON);
	ПараметрыЗапроса.Вставить("ПараметрыПреобразованияJSON", ПараметрыПреобразованияJSON);
