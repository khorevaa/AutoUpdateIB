...
Процедура ОтправитьСообщение(Знач ДатаСобытия, Знач ФорматированноеСообщение)
	...
	Если ЗначениеЗаполнено(КлючСоответствия) Тогда
		СообщениеRMQ.Параметр("КлючСоответствия", Строка(КлючСоответствия));
	КонецЕсли;

	СообщениеRMQ.ТочкаОбмена = ТочкаОбмена;
	СообщениеRMQ.КлючМаршрутизации = Строка(КлючМаршрутизации);
	СообщениеRMQ.ДанныеСообщения = ФорматированноеСообщение;

	Попытка
		КлиентRMQ.ОтправитьСообщениеRMQ(СообщениеRMQ);
	Исключение
		Сообщить(СтрШаблон("Ошибка отправки сообщения в RMQ: <%1>", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке())) , СтатусСообщения.Важное);
	КонецПопытки;
