...
Процедура ПриСозданииОбъекта(Знач ПЛог, Знач ПВыводитьВремяВыполнения = Истина)
	Лог = ПЛог;
	ИндексЗадач = Новый Соответствие();

	Если ПВыводитьВремяВыполнения = Неопределено Тогда
		ВыводитьВремяВыполнения = Истина;
	Иначе
		ВыводитьВремяВыполнения = ПВыводитьВремяВыполнения;
	КонецЕсли;
...
Процедура Трассировать(Знач ИмяЗадачи, Знач Сообщение = "", Знач Статус = "do", Знач Ошибка = Неопределено,  ДопПоля = Неопределено) Экспорт
	...
	ИначеЕсли ВыводитьВремяВыполнения Тогда
		ОписаниеТрассировки.Вставить("duration", ТаймерЗадачи.ВремяЗамера());
		ОписаниеТрассировки.Вставить("timer", ТаймерЗадачи.Длительность());
	КонецЕсли;
