...
Процедура СоздатьРезервнуюКопию() Экспорт
	...
	Исключение
		...
		ТрассировкаЗадач.Трассировать("Создание резервной копии", "Ошибка создания резервной копии информационной базы", "error", Инфо);
...
Процедура ВосстановитьРезервнуюКопию() Экспорт
	...
	Исключение
		Инфо = ИнформацияОбОшибке();
		ТрассировкаЗадач.Трассировать("Восстановление из резервной копии", "Ошибка восстановления информационной базы из резервной копии", "error", Инфо);
		ВызватьИсключение КраткоеПредставлениеОшибки(Инфо);
