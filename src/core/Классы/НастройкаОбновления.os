...
Функция КонструкторПараметровОбновленияКонфигурации()
	...
	КонструкторПараметров
						...
						.ПолеБулево("ВозвратККонфигурацииБазыДанных rollback-cf", Ложь)
...
Процедура ПолучитьФайлЕслиТребуется(Параметры, КлючСвойства, КлючСвойстваЗаписи)
	...
	ПутьКФайлуХеша = КешФайлов.ПолучитьФайл(ХешФайла);
		
	Если НЕ ЗначениеЗаполнено(ПутьКФайлуХеша) Тогда
		...
		ПутьКФайлу = ПутьКФайлуВременный;

		Лог.Поля("ХешФайла", ХешФайла, "ПутьКФайлу", ПутьКФайлу, "КлючСвойства", КлючСвойстваЗаписи).Отладка("Получен файл в кеш");
	
	Иначе

		ФайлХеша = Новый Файл(ПутьКФайлуХеша);
		
		ВременныйФайл = ВременныеФайлы.НовоеИмяФайла(ФайлХеша.Расширение);

		КопироватьФайл(ПутьКФайлуХеша, ВременныйФайл);
	
		ПутьКФайлу = ВременныйФайл;
	
		Лог.Поля("ХешФайла", ХешФайла, "ПутьКФайлу", ПутьКФайлу).Отладка("Файл найден в кеше и скопирован во временный каталог");
