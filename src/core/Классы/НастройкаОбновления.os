...
Перем ФайлОбновления;
Перем ПолучениеФайлаПередОбновлением;
Перем ПолучениеФайлаПослеОбновления;

Перем КаталогКешаФайлов;

Перем ПроцессорВыводаЛогаВТаблицуЗначений;

Функция ПутьКОбработкеПередОбновлением() Экспорт
	Возврат ПолучениеФайлаПередОбновлением.Получить();
КонецФункции

Функция ПутьКОбработкеПослеОбновления() Экспорт
	Возврат ПолучениеФайлаПослеОбновления.Получить();
КонецФункции
...
Функция ПутьКОбновлению() Экспорт
	Возврат ФайлОбновления.Получить();
КонецФункции
...
Процедура ПрочитатьНастройкиПоВерсии10(ИндексЗначений)
	...
	НастройкаПолученияФайлаПередОбновлением = ЗначениеИзИндекса(ИндексЗначений, "ПередОбновлением", Новый Соответствие);
	НастройкаПолученияФайлаПослеОбновления = ЗначениеИзИндекса(ИндексЗначений, "ПослеОбновления.", Новый Соответствие);
...
Функция КонструкторПараметровВерсии10()
	...
	ПодключениеКФайловойбазе = Конструктор.НовыеПараметры("ПодключениеКФайловойбазе");
	ПодключениеКФайловойбазе
		.ПолеСтрока("ПутьККаталогу path dir", "")
		;

	ПодключениеКСервернойбазе = Конструктор.НовыеПараметры("ПодключениеКСервернойбазе");
	...
	Конструктор
		...
		.ПолеОбъект("ПередОбновление pre-update", ПолеПолученияФайлов)
