...
Функция ВыполнитьОбновление(НастройкаОбновления) Экспорт
	...
	ТрассировкаЗадач.Трассировать("Работа с информационной базой", "Начало работы с информационной базой");
	...
	Если РезультатОбновления.Выполнено Тогда
		ТрассировкаЗадач.Трассировать("Работа с информационной базой", "Успешно завершена работа с информационной базой", "success");
	Иначе
		ТрассировкаЗадач.Трассировать("Работа с информационной базой", "Работа с информационной базой завершена с ошибкой", "error", ОписаниеОшибки.Ошибка);
	КонецЕсли;
...
Процедура ОбработкаОшибкиВыполнения(ЗадачаБизнесПроцесса, ПродолжитьВыполнение, СтандартнаяОбработка) Экспорт
	...
	ОписаниеОшибкиЗадачи = ЗадачаБизнесПроцесса.ПолучитьОписаниеОшибки().Ошибка;
	
	Лог.Поля("ИмяЗадачи", ЗадачаБизнесПроцесса.Наименование(), "ОписаниеОшибки", ОписаниеОшибкиЗадачи).Отладка("Обработка ошибки выполнения задачи");
	
	КонтекстБизнесПроцесса = БизнесПроцессЗадачи.ПолучитьКонтекст();
	
	Если Не ОшибкаТребуетВосстановленияРезервнойКопии(КраткоеПредставлениеОшибки(ОписаниеОшибкиЗадачи)) Тогда
...
Функция ОшибкаТребуетВосстановленияРезервнойКопии(Знач ОписаниеОшибкиЗадачи)
	...
	ТребуетВосстановления = Истина;

	МассивОшибок = СписокОшибокНеТребующихВосстановленияРезервнойКопии();

	Для каждого ТекстОшибки Из МассивОшибок Цикл
		
		Найдено = СтрНайти(ОписаниеОшибкиЗадачи, ТекстОшибки) > 0;

		Если Найдено Тогда
			ТребуетВосстановления = Ложь;
			Прервать;
		КонецЕсли;

	КонецЦикла;

	Возврат ТребуетВосстановления;
...
Функция СписокОшибокНеТребующихВосстановленияРезервнойКопии()
	...
	МассивОшибок.Добавить("Ошибка обновления расширений");
