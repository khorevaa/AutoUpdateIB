...
Функция НастроитьКонфигуратор(НастройкаОбновления) Экспорт
	...
	Конфигуратор.УстановитьКонтекст(НастройкаОбновления.СтрокаПодключения,
				...
				АвторизацияВИнфомарционнойБазе.Паоль);
...
Процедура ВыполнитьОбновление()
	...
	Если НастройкаОбновления.ЗагрузкаКонфигурацииВместоОбновления Тогда
		...
		РабочийКонфигуратор.ЗагрузитьКонфигурациюИзФайла(ЗагрузитьКонфигурациюИзФайла);
...
Процедура ПередОбновлением()
	...
	Попытка
		Лог.Информация("Выполнение перед обновлением информационной базы");	
		ВыполнитьОбработку(НастройкаОбновления.ПутьКОбработкеПередОбновлением);
	Исключение
		Лог.КритичнаяОшибка("Выполнение перед обновлением информационной базы: %1", ОписаниеОшибки());
		ОшибкаПроцессаВыполнения = Истина;
	КонецПопытки;
...
Процедура ПослеОбновления()
	...
	Попытка
		Лог.Информация("Выполнение после обновления информационной базы");	
		ВыполнитьОбработку(НастройкаОбновления.ПутьКОбработкеПослеОбновлени);
	Исключение
		Лог.КритичнаяОшибка("Выполнение после обновления информационной базы: %1", ОписаниеОшибки());
		ОшибкаПроцессаВыполнения = Истина;
	КонецПопытки;
...
Процедура ВыполнитьОбработку(Знач ПутьКФайлуОбработки)
	...
	ФайлОбработки = Новый Файл(ПутьКОбновлению);

	Если НЕ ФайлОбработки.Существует() Тогда
		ВызватьИсключение НоваяИнформацияОбОшибке("Файл обработки <%1> не найден", ФайлОбновления.ПолноеИмя);
	КонецЕсли;
