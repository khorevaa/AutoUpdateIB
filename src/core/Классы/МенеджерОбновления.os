...
Функция ВыполнитьОбновление(НастройкаОбновления) Экспорт
	...
	Если РезультатОбновления.Выполнено Тогда
		Лог.Поля("duration", Таймер.ВремяЗамера()).Информация("Успешно завершена работа с информационной базой");
	Иначе
		Лог.Поля("duration", Таймер.ВремяЗамера(), "error", ОписаниеОшибки.Ошибка).КритичнаяОшибка("Работа с информационной базой завершена с ошибкой");
	КонецЕсли;
