...
Процедура УстановитьФорматВыводаЛогов(Знач ОбъектФорматирования) Экспорт
	...
	Для каждого ИмяЛога Из МассивЛогов Цикл
		ЛогПриложения = Логирование.ПолучитьЛог(ИмяЛога);
		ЛогПриложения.УстановитьРаскладку(ОбъектФорматирования);
	КонецЦикла;
...
Процедура ДобавитьВыводЛогаВФайл(Объект) Экспорт
	
	Если Не ЗначениеЗаполнено(ФайлОбщегоЛога) Тогда
		Возврат;
	КонецЕсли;
	
	Объект.ДобавитьСпособВывода(ФайлЖурнала);
	
	// Если ВыводитьЛогВКонсоль Тогда
	
	ВыводВКонсоль = Новый ВыводЛогаВКонсоль();
	Объект.ДобавитьСпособВывода(ВыводВКонсоль);
