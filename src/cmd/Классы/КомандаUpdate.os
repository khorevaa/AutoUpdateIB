...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	МассивОшибокВыполнения = Новый Массив();
	Для каждого НастройкаОбновления Из МассивНастроекОбновления Цикл

		// Лог.Информация("-----------------------------------------");	
		Лог.Поля("ИндексНастройки", МассивНастроекОбновления.Найти(НастройкаОбновления)).Информация("Запуск по настройке <%1>", НастройкаОбновления["name"]);	
		ВыполнитьОбновлениеПоНастройке(НастройкаОбновления, КаталогКешаФайлов, ПользовательскимиПараметры, МассивОшибокВыполнения);
		// Лог.Информация("-----------------------------------------");	
		
	КонецЦикла;

	Если МассивОшибокВыполнения.Количество() > 0  Тогда
		
		Для каждого ЭлементМассив Из МассивОшибокВыполнения Цикл
			
			ИндексНастройки = МассивНастроекОбновления.Найти(ЭлементМассив.НастройкаОбновления);
			Лог.Поля("ИндексНастройки", ИндексНастройки, "ОписаниеОшибки", ЭлементМассив.ОписаниеОшибки).Предупреждение("Ошибка выполнения по настройке <%1>", НастройкаОбновления["name"]);	

		КонецЦикла;
		
		Лог.Поля("КоличествоОшибок", МассивОшибокВыполнения.Количество()).Предупреждение("Работа завершена с ошибками");
...
Процедура ВыполнитьОбновлениеПоНастройке(НастройкаОбновленияБазы, КаталогКешаФайлов, ПользовательскимиПараметры, МассивОшибокВыполнения)
	...
	Иначе
		...
		ОписаниеОшибкиВыполнения = КраткоеПредставлениеОшибки(РезультатВыполнения.ОписаниеОшибки);
		Лог.Поля("ОписаниеОшибки", ОписаниеОшибкиВыполнения).КритичнаяОшибка("Задача не выполнена");
		
		МассивОшибокВыполнения.Добавить(Новый Структура("НастройкаОбновления, ОписаниеОшибки", НастройкаОбновленияБазы, ОписаниеОшибкиВыполнения));
