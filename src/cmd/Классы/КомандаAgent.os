...
Процедура ОписаниеКоманды(Команда) Экспорт
	...
	Команда.Опция("report-queue", "", "имя очереди RabbitMQ для логов агента и рабочих процессов")
				.ВОкружении("RMQ_REPOT_QUEUE");
	...
	Команда.Опция("t queue-timer", 600, "таймер опроса сервера очереди");

	Команда.Опция("id", Новый УникальныйИдентификатор(), "идентификатор агента").ТСтрока();
	...
	Команда.Опция("T worker-timeuot", 60*60*6, "таймаут перезапуска рабочего процесса агента при зависании");
...
Процедура ВыполнитьКоманду(Знач Команда) Экспорт
	...
	ИмяОчередиЛогов = Команда.ЗначениеОпции("report-queue");;
	...
	ИдентификаторАгента =  Команда.ЗначениеОпции("id");
	ВыводЛогаВКонсоль =  Команда.ЗначениеОпции("log-console");
	...
	НастройкаRMQ.Вставить("ИмяОчередиЛогов", ИмяОчередиЛогов);

	МенеджерРабочийПроцессов = Новый АгентОбновления(ИдентификаторАгента);
	МенеджерРабочийПроцессов.УстановитьОткладку(Отладка);
	МенеджерРабочийПроцессов.НастроитьRMQ(НастройкаRMQ);
	МенеджерРабочийПроцессов.УстановитьКоличествоРабочихПроцессов(МаксимальноеКоличествоРабочихПроцессов);	
	МенеджерРабочийПроцессов.УстановитьТаймерОпроса(ТаймерОпросаОчереди);
	МенеджерРабочийПроцессов.УстановитьТаймаутРабочегоПроцесса(ТаймаутРабочегоПроцесса);
	МенеджерРабочийПроцессов.УстановитьРабочийКаталогПроцессов(РабочийКаталогПроцессов);
	МенеджерРабочийПроцессов.УстановитьВыводЛогаВКонсоль(ВыводЛогаВКонсоль);
